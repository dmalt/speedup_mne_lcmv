...
from mne.minimum_norm.inverse import (
    combine_xyz, _check_reference, _check_depth
)
from mne.cov import compute_covariance
from mne.source_estimate import _make_stc, _get_src_type
from mne.utils import (logger, verbose, warn, _validate_type, _reg_pinv,
                       _check_channels_spatial_filter, _check_option)
from mne.utils import _check_one_ch_type, _check_rank, _check_info_inv
...
from compute_beamformer_dmalt import _compute_beamformer_dmalt
from time import time  # dmalt


@verbose
def make_lcmv_dmalt(info, forward, data_cov, reg=0.05, noise_cov=None, label=None,
                    pick_ori=None, rank='info', weight_norm='unit-noise-gain',
                    reduce_rank=False, depth=None, verbose=None, is_use_jit=False):
    ...
    return filters, t2 - t1


@verbose
def make_lcmv_mne(info, forward, data_cov, reg=0.05, noise_cov=None,
                  label=None, pick_ori=None, rank='info',
                  weight_norm='unit-noise-gain', reduce_rank=False,
                  depth=None, verbose=None):
    """Compute LCMV spatial filter.
    ...
    W = _compute_beamformer(G, Cm, reg, n_orient, weight_norm, pick_ori,
                            reduce_rank, rank_int, inversion='matrix', nn=nn,
                            orient_std=orient_std)
    t2 = time()
